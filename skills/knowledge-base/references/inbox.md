...
1. **Parent:** scans `aa-inbox/`, lists items, detects companion `.txt` files, and runs the scripted duplicate check per item: `kb-dup-check "<candidate stem>" [--url <url>]`, passing the best available filename stem (the dropped name, or the prospective renamed stem if metadata is already evident) and the item's URL when known. `DUP` flags a probable duplicate (Step 5 leaves it in the inbox and reports); `NEAR` is a fuzzy hit to surface for the user's call; `ERROR` means the check could not complete, so treat the item as unverified and say so. Never eyeball-compare against a whole-file `index.md` read.
